 ...
 *
 * This program now uses the BLKRRPART ioctl, so you need Linux 0.99 or
 * above.  --Adam J. Richter 1/1/93
 *
...
char    *disk_device = NULL,		/* hda, unless specified */
...
void fatal(enum failure why)
{
        ...
        switch (why) {
                case usage: message = "Usage: fdisk [-l] [-v] [-r reboot_flag_file] [/dev/hdx]\n";
                        break;
                case unable_to_open:
...
#if 0	/* Remove stupid warnings. --AJR */
		/* This warning probably takes about as much disk space to implement
		   it saves (an average of 512/2 = 256 bytes). */
                if ((limit = p->nr_sects) & 1)
                        printf("Warning: partition %d has an odd "
                                "number of sectors.\n", n + 1);
		/* This warning applies only to the old minix filesystem, which
		   is not used in the installation process. */
                if (limit > 131070)
...
void write_table()
{
	...
	if( ioctl( fd, BLKRRPART, NULL ) < 0 ) {
	    ...
	    if ( errno != EBUSY ) {
		perror("Failed to load new partition table back from disk");
	    }
	    if( reboot_file ) {
		if ((reboot_file_fd = creat( reboot_file, 0444 )) < 0 ) {
		    perror( reboot_file );
		}
		else {
		    close( reboot_file_fd );
		}
	    }
	    if( !reboot_file || reboot_file_fd < 0 ) {
...
void main(int argc, char **argv)
{
        ...
        if (argc > 3)
                fatal(usage);
        disk_device = argc > 1 ? argv[argc - 1] : DEFAULT_DEVICE;
        get_boot();
