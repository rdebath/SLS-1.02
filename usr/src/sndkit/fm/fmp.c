...
static int num_cells, perc_mode, perc_chan = 15;
...
void prepare_for_CMF(void)
{
	...
	fileseek(0);
	perc_chan = 9;	/* Ch#10 used for percussive sounds in CMF */
...
int main(int argc,char **argv)
{
	...
	while((opt = getopt(argc,argv,"qv:p:")) != -1) {
	    switch(opt) {
	...
	printf("Nr. voices %d, perc. mode %d\n", num_cells, perc_mode);
