...
#define Tk_FreeGC(dpy,bitmap)	/* as nothing -- AJR */
#define Tk_FreeCursor(dpy,bitmap)	/* as nothing -- AJR */
    ...
    Line *linePtr;
{
    ...
    if (linePtr->gc != None) {
        printf ("AJR graph3 Freegc, linePtr->gc 0x%x\n", linePtr->gc);
    ...
    Graph *graphPtr;
{
	...
	if (!NULLSTR(tagPtr->text) || (tagPtr->bitmap != None)) {
		...
		Tk_SizeOfBitmap(Tk_Display(graphPtr->tkwin),
					   tagPtr->bitmap, &w, &h);
    ...
    ClientData clientData;
{
    ...
    if (graphPtr->gc != None) {/* Graphics context of graph */
        printf ("AJR graph6 Freegc, graphPtr->gc 0x%x\n", graphPtr->gc);
