...
void hfont_print(f, x, y, angle, s) hfont_scaled *f; int *x, *y, angle; unsigned char *s;
{
  ...
  while (l=*s++)
  {
    ...
    while (num-->0)
    {
      ...
      else
      {
        xc=(c->x * cosx - c->y * siny)>>10;
        yc=(c->x * sinx + c->y * cosy)>>10;
        for (i=0; i<=f->x_overlay; i++) for (j=0; j<=f->y_overlay; j++) bitline(cx+i,cy+j,i+SINGLE(*x)+xc,SINGLE(*y)-yc+j);
      }
      cx=SINGLE(*x)+xc; cy=SINGLE(*y)-yc; c++;
    }
    *x=*x+((width*cosx)>>10);
