...
static GlyphIndex prev_forced_break(
    ...
    while (i >= 0) {
        CompositionComponent& comp = component->item_ref(i);
        if (comp.glyph_ != nil) {
            Requisition r;
...
static GlyphIndex fill_arrays (
    ...
    while (last < count) {
        ...
        CompositionComponent& comp = component->item_ref(last);
        if (comp.glyph_ != nil) {
            Requisition r;
            comp.glyph_->request(r);
    ...
    if (damaged_) {
	    ...
	    GlyphIndex b;
            for (b = 0; b < count && (b+break_index) < break_count; ++b) {
                Break& br = breaks_->item_ref(b + break_index);
...
void Composition::pick(
	...
	GlyphIndex b_count = breaks_->count();
		 ...
		 */
		Break& br = breaks_->item_ref(b_index);
...
Glyph* LRComposition::make(Break& nb) {
    const LayoutKit& layout = *LayoutKit::instance();
    Glyph* glyph = layout.hbox_first_aligned(nb.last_ - nb.first_ + 5);
...
Glyph* TBComposition::make(Break& nb) {
    const LayoutKit& layout = *LayoutKit::instance();
    Glyph* glyph = layout.vbox_first_aligned(nb.last_ - nb.first_ + 5);
