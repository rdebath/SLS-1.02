#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)cnvs_input.c 20.48 91/09/14";
#endif
#endif
...
*/
                XSetICValue(ic, XNClientWindow, xv_xid(info), 0);
                XSetICFocus(ic);
		frame = (Frame) xv_get(canvas_public, WIN_FRAME);
		(void) xv_set(frame, WIN_IC, ic, 0);

		if (canvas->pe_cache->chg_length)
		    canvas_text_draw(ic, canvas_public, canvas->pe_cache);
...
/*
	if ((ic = (XIC)xv_get(window_public, WIN_IC)) &&
	    xv_get(window_public, WIN_USE_IM) &&
            xv_get(window_public, WIN_CONSUME_EVENT, WIN_ASCII_EVENTS)) {
	    XUnSetICFocus(ic);
	}
*/
