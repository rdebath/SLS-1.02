#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_filter.c 20.32 91/09/14";
#endif
#endif
 ...
 */
{
    ...
    dummy[0] = NULL;
    textsw_init_selection_object(folio, fill_in, dummy, 0, FALSE);
#else
...
#define	NR_LAST_PLUS_ONE	1
...
#ifdef OW_I18N
    /* 
     *	Half-sized for OW_I18N because we assume mb may be twice
     *	as long as wchar_t.
     */
    unsigned long   buffer_and_slop[(PIPSIZ + SLOP_SIZE) /
				    (sizeof(unsigned long) / sizeof(CHAR)) / 2];
#else
    unsigned long   buffer_and_slop[(PIPSIZ + SLOP_SIZE) /
...
#ifdef OW_I18N
    int		    unconverted_mb_count = 0; /* Number of unconverted bytes
						 from last read */
    char	    unconverted_mb_bytes[10 * 10];
					      /* Unconverted bytes from last
						 read */
			/* Assume max 10 bytes/char, max 10 chars */
    CHAR	    wc_buf[sizeof(buffer_and_slop)];
					      /* buffer for converting mb read
						 from filter to wchar_t */
#endif
...
#ifdef OW_I18N
	/*
	 * The following code is pretty ugly, because we don't know if a buffer
	 * read from the filter contains complete mb char sequences. The last
	 * sequence may be incomplete.  If it is, it is saved in 
	 * unconverted_mb_bytes[] and unconverted_mb_count is set to the
	 ...
	 * buffer.
	 */
	    if (unconverted_mb_count) {
		XV_BCOPY(unconverted_mb_bytes, buffer, unconverted_mb_count);
	    }
	/*
	 * Read data from the filter into the buffer after the leftovers.
	 */
	    bytes_read = read(filter_output, 
			      (char *) buffer + unconverted_mb_count,
			      sizeof(buffer_and_slop) - SLOP_SIZE - 
			          unconverted_mb_count);
	    bytes_read += unconverted_mb_count;
	    unconverted_mb_count = 0;
	    temp_bytes_read = bytes_read;
	    buffer[bytes_read] = NULL;
	    wc_len = my_mbstowcs(wc_buf, buffer, &temp_bytes_read);

	/*
	 * If an incomplete mb sequence was left untranslated, copy it into
	 * unconverted_mb_bytes[] and set unconverted_mb_count to the the
	 * count of bytes left over.
	 */
	    if (temp_bytes_read != bytes_read) {
		ASSUME(bytes_read - temp_bytes_read <= 100);
		XV_BCOPY(buffer + temp_bytes_read, unconverted_mb_bytes,
			bytes_read - temp_bytes_read);
		unconverted_mb_count = bytes_read - temp_bytes_read;
	    }
#else OW_I18N
	...
	if((filter_input >= 0) && (FD_ISSET(filter_input, &writefds)))
	{
...
#ifdef OW_I18N
	    temp_wc_buf = MALLOC(to_write + 1);
	    next = es_read(folio->views->esh, to_write, temp_wc_buf, &to_write);	    
            if (to_write < (sizeof(buffer_and_slop) - SLOP_SIZE))
	        temp_wc_buf[to_write] = NULL;
	    else /* BIG BUG:  This is an error case. */
	        temp_wc_buf[(sizeof(buffer_and_slop) - SLOP_SIZE) - 1] = NULL;

	    if (READ_AT_EOF(first, next, to_write)) {
		first = last_plus_one;
		if (temp_wc_buf)
		    free((char *)temp_wc_buf);
	    } else {
		char 	   *mb_buf;

		ASSUME(to_write <= PIPSIZ);
		mb_buf = wcstombsdup(temp_wc_buf);
		if (temp_wc_buf)
		    free((char *)temp_wc_buf);
		written[0] = write(filter_input, mb_buf, strlen(mb_buf));
		if (mb_buf)
		    free(mb_buf);
		...
		/*
		 * This shouldn't really be an error, but c'est la guerre.
		 * This'll do for alpha.
		 * We should really keep track of how much of the mb we
		 * wrote and write the rest the next time we can write
		 * before we convert any more wchar_t to mb.
		 */
			    goto ErrorReturn;
		    first = next;
		}
