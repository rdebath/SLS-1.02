#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_move.c 20.77 91/09/14";
#endif
#endif
...
#define _TEXTSW_CONTENTS	TEXTSW_CONTENTS_WCS
#else
#define _TEXTSW_CONTENTS	TEXTSW_CONTENTS
#endif
    ...
    register Event *ie;
{
    ...
    xv_get(textsw, _TEXTSW_CONTENTS, first - 1, buf, 2);
    if (buf[0] == ' ') {
	xv_get(textsw, _TEXTSW_CONTENTS, last_plus_one, buf, 1);
	if (buf[0] == ' ') {
	    last_plus_one++;
	}
    }
    ...
    xv_get(textsw, _TEXTSW_CONTENTS, pos - 1, buf, 2);
    if (buf[1] == ' ') {
	if (buf[0] != ' ') {
	    /* add leading space */
    ...
    register Event *ie;
{
    ...
    xv_get(textsw, _TEXTSW_CONTENTS, position, buf, 1);
    if (buf[0] == ' ') {
	xv_get(textsw, _TEXTSW_CONTENTS, position - 1, buf, 1);
	if (buf[0] != ' ') {
    ...
    Es_index        first, last_plus_one;
{
    ...
    /* if spaces on either side, collapse them */
    xv_get(textsw, _TEXTSW_CONTENTS, first - 1, first_buf, 1);
    if (first_buf[0] == ' ') {
	xv_get(textsw, _TEXTSW_CONTENTS, last_plus_one, last_buf, 1);
	if (last_buf[0] == ' ')
	    shift_left = TRUE;
    }
    xv_get(textsw, _TEXTSW_CONTENTS, last_plus_one, last_buf, 1);
    ...
    int             format;
{
	...
	if (type == (Atom)xv_get(server, SERVER_ATOM, "INCR")) {
		...
		string = (char *)xv_realloc(string, strlen(string) + length);
...
#undef _TEXTSW_CONTENTS		/* OW_I18N */
