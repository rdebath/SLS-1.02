#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_file.c 20.55 91/09/14";
#endif
#endif
...
#ifdef OW_I18N
    if (folio->temp_mark)
	textsw_implicit_commit(VIEW_FROM_FOLIO_OR_VIEW(folio));
#endif
    ...
    int             print_error_msg;
{
	...
	wc_count = my_mbstowcs(buf_ws, buf, &temp_count);
	if (temp_count != count) {
	    /* re-read the incomplete mb character */
	    /* new_pos = lseek(fd, temp_count - count, L_INCR); */
            /* According to the manual, SEEK_foo are used for whence.
             * I picked the one with the same value as L_INCR which
             * does not exist on SVR4.
             */
    ...
    int             locx, locy;
{
    ...
    (void) mbstowcs(filename, filename_mb, MAXNAMLEN);
#endif
...
#ifdef OW_I18N
    if (folio->temp_mark)
	textsw_implicit_commit(VIEW_ABS_TO_REP(abstract));
#endif
    if (preserve_memory) {
