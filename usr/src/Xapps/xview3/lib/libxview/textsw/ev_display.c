#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ev_display.c 20.46 91/09/14";
#endif
#endif
    ...
    int             x, y;
{
    ...
    GC         *gc_list;
    GC          gc;
    static Xv_opaque last_server = 0;
    ...
    unsigned        	    type;
{
    ...
    register Ev_mark 	    first_mark, last_mark;
    Ev_mark         	    to_use;
    Ev_mark_object	    temp[2];
    Ev_finger_handle 	    finger;
    
    first_mark = &first_mark_obj;
    last_mark = &last_mark_obj;
    EV_INIT_MARK(*first_mark);
    EV_INIT_MARK(*last_mark);
    ev_add_finger(&chain->fingers, first, 0, first_mark);
    ev_add_finger(&chain->fingers, last_plus_one, 0, last_mark);
...
ev_clear_pre_edit_region(chain, first, last_plus_one, type)
    Ev_chain        	    chain;
    register Es_index 	    first, last_plus_one;
    unsigned        	    type;
{
    ...
    ev_remove_op_bdry(&private->op_bdry, first,
		      type, EV_BDRY_TYPE_ONLY);
    ev_remove_op_bdry(&private->op_bdry, last_plus_one,
		      type | EV_BDRY_END, EV_BDRY_TYPE_ONLY);
}
#endif
