#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_selsvc.c 20.53 91/09/14";
#endif
#endif
...
#ifdef OW_I18N
char           *shelf_name_mb = "/tmp/textsw_shelf";
#else
...
#ifdef OW_I18N    
    Es_index        current_mb, first_mb, last_plus_one_mb;  
#endif
    unsigned        span_level;
}               Continuation_object;
...
#ifdef OW_I18N 
#define SHELF_NAME_LENGTH		50
    static CHAR	    shelf_name[SHELF_NAME_LENGTH];
#endif
	...
	return;
#ifdef OW_I18N
    if (STRLEN(shelf_name) == 0)
	(void) mbstowcs(shelf_name, shelf_name_mb, SHELF_NAME_LENGTH);
#endif
    ...
    register Seln_request *reply;
{
	...
	switch ((Seln_attribute) (*attr)) {
	  ...
	  case SELN_REQ_CONTENTS_ASCII: {
		...
		to_read = temp_len = selection->last_plus_one - selection->first;
		if (extra_ptr) {
	     ...
	     */
	    temp_len = to_read;
	    selection->buf_len = my_mbstowcs(selection->buf, temp_ptr, &temp_len);
	    if (extra_ptr) {
	        free(extra_ptr);
	        extra_ptr = NULL;
	    }
...
#ifdef OW_I18N
    count = (continued) ? count + TCAR_CONTINUED : count;
    return (count);
#else
...
#ifndef notdef		/* This routine is not being used.  nng */
char           *
...
#ifdef OW_I18N
pkg_private	Es_index
textsw_get_char_in_multibyte (first, last_plus_one, buffer, buffer_size,
...
#define	    LARGE_BUF_SIZE	1024
    ...
    static char	   *temp_mb;
    static CHAR	   *temp_wc;
    static int	    temp_mb_len, temp_wc_len;
    int	    	    temp_size;
    Es_index	    num_of_byte;
    int		    continued;

    if (buffer)
        temp_size = (buffer_size / sizeof(CHAR))  - 1;
    else
        temp_size = (last_plus_one - first) + 1;
    if (temp_size > temp_wc_len) {
        if (temp_wc)
            free(temp_wc);
        temp_wc_len = temp_size;
        temp_wc = MALLOC(temp_wc_len);
    }
    *char_count = textsw_copy_wcs_reply(first,
    			    last_plus_one, temp_wc,
    			    temp_wc_len,
    			    flags, (caddr_t) esh_to_use);
    if (continued = (*char_count >= TCAR_CONTINUED))
	*char_count -= TCAR_CONTINUED;
    temp_wc[*char_count] = NULL;

    if (!buffer) {
        if (((wslen(temp_wc) * MB_CUR_MAX) + 1) > temp_mb_len) {
            if (temp_mb)
                free(temp_mb);
            temp_mb_len = ((wslen(temp_wc) * MB_CUR_MAX) + 1);
            temp_mb = malloc(temp_mb_len);
        }
        num_of_byte = wcstombs(temp_mb, temp_wc, temp_mb_len);
    } else
        num_of_byte = wcstombs(buffer, temp_wc, buffer_size);

#ifdef notdef
    printf("ws len =%d ", wslen(temp_wc));
    printf("count = %d, num_of_byte = %d\n", *char_count, num_of_byte);
#endif

    if (temp_mb_len > LARGE_BUF_SIZE) {
       /*
        * If the temp buffers are bigger than 1K,
        * then don't keep it around.
        */
       free(temp_mb);
       temp_mb = NULL;
       temp_mb_len = 0;
    }
    if (temp_wc_len > LARGE_BUF_SIZE) {
       /*
        * If the temp buffers are bigger than 1K,
        * then don't keep it around.
        */
...
static void
textsw_get_first_mb_and_last_plus_one_mb(folio, cont_data)
    Textsw_folio 	folio;
    Continuation 	cont_data;
{
    int			dummy;

    if ((cont_data->first != ES_INFINITY) &&
        (cont_data->last_plus_one != ES_INFINITY)) {
        cont_data->first_mb = textsw_get_char_in_multibyte (
	         0, cont_data->first, NULL, 0, &dummy,
	         TCARF_ESH,
	         (cont_data->type == EV_SEL_SHELF)
		     ? (caddr_t)folio->trash : (caddr_t)folio->views->esh); 

        cont_data->last_plus_one_mb = (textsw_get_char_in_multibyte (
	         cont_data->first, cont_data->last_plus_one, NULL, 0, &dummy,
	         TCARF_ESH,
	         (cont_data->type == EV_SEL_SHELF)
		     ? (caddr_t)folio->trash : (caddr_t)folio->views->esh) +
		        cont_data->first_mb);
    } else {
      cont_data->last_plus_one_mb = cont_data->first_mb =  ES_INFINITY;
    }
}
#endif OW_I18N
...
#ifdef OW_I18N 	
      case SELN_REQ_BYTESIZE: {
        if ((cont_data->first_mb == 0) && (cont_data->last_plus_one_mb == 0)) {
            textsw_get_first_mb_and_last_plus_one_mb(textsw, cont_data);
        }
	...
	*context->response_pointer++ =
	...
	goto Return;
      case SELN_REQ_CONTENTS_ASCII: {
	    ...
	    byte_count = textsw_get_char_in_multibyte(current_first, 
	    			    cont_data->last_plus_one, 
      ...
      case SELN_REQ_CONTENTS_WCS:{
	    ...
	    count = textsw_copy_wcs_reply(
				    current_first, cont_data->last_plus_one,
			   	    context->response_pointer,
				    max_length / sizeof(CHAR), TCARF_ESH,
				    (caddr_t) esh_to_use);
	...
	*context->response_pointer++ =
	...
	goto Return;
      ...
      case SELN_REQ_FILE_NAME:{
      	    int             continued, count, byte_count;
	    ...
	    if (textsw_file_name(textsw, &name_wcs) != 0) {
		*context->response_pointer++ = 0; /* Null terminate value */
		goto Return;
	    }
	    /*
		...
		? 0 : (cont_data->current);
	    str_last_plus_one = STRLEN(name_wcs);
	    temp_name = MALLOC(str_last_plus_one +
			       sizeof(*context->response_pointer));
	    count = textsw_copy_wcs_reply(
			str_first, str_last_plus_one, temp_name,
			str_last_plus_one + sizeof(*context->response_pointer),
			TCARF_STRING, name_wcs);
	    
	    if (continued = (count >= TCAR_CONTINUED))
		count -= TCAR_CONTINUED;
	    temp_name[count] = NULL;
	    byte_count = wcstombs((char *)context->response_pointer,
