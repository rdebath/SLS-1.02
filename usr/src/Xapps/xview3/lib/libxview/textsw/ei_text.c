#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ei_text.c 20.46 91/09/14";
#endif
#endif
    ...
    register int    tab_width;
{
    ...
    if (x_font_info->per_char)  {
        private->tab_pixel =
	    x_font_info->per_char['m' - x_font_info->min_char_or_byte2].width * tab_width;
    }
    else  {
        private->tab_pixel =
    ...
    Ei_handle       eih;
{
    ...
    XFontSet	    font_set = (XFontSet)xv_get((Xv_opaque)private->font, FONT_SET_ID);
    char	    localename[256];
    ...
    register Xv_Font font;
{
    ...
    private->pf_font = tempPf = (Pixfont *)xv_get((Xv_opaque)font, FONT_PIXFONT);
    ...
    Ei_handle       eih;
{
    ...
    return (xv_get((Xv_opaque)private->font, FONT_DEFAULT_CHAR_HEIGHT));
}
 ...
 */
{
     ...
     */
    bounds_bottom = SSub(SAdd(temp, xv_get((Xv_opaque)private->font, FONT_DEFAULT_CHAR_HEIGHT)), 1);
...
Rescan:
	if (c == SPACE) {
	    ...
	    tempPf = (Pixfont *)xv_get((Xv_opaque)private->font, FONT_PIXFONT);
#endif
#ifdef OW_I18N
...
/*
	    special_char = (c < ' ' || c >= 128 && c <= 159) ? c + 64 : '?';
*/
	    special_char = ISCNTRL(c) ? c + 64 : '?';
	    c = '^';
	    goto Rescan;
	}
Skip_pc_pr_tests:
	...
	switch ((Ei_attribute) * attributes) {
	  ...
	  case EI_LOCALE:
		...
		private->locale_is_ale =
		    ...
		    || (strcmp(private->current_locale, "korean") == 0)
