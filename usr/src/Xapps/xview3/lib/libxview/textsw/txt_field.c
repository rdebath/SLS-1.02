#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_field.c 20.23 91/09/14";
#endif
#endif
    ...
    unsigned        field_flag;
{
...
#ifdef OW_I18N
    CHAR           *start_sym_wc, *end_sym_wc;	/* begin and end pattern */

    start_sym_wc = mbstowcsdup(start_sym);
    start_sym_len = STRLEN(start_sym_wc);
    end_sym_wc = mbstowcsdup(end_sym);
    end_sym_len = STRLEN(end_sym_wc);
#endif
	...
	(void) textsw_match_field(folio, first, last_plus_one,
#ifdef OW_I18N
				  start_sym_wc, start_sym_len,
				  end_sym_wc, end_sym_len, field_flag, TRUE);
#else
	...
	(void) textsw_match_field(folio, first, last_plus_one,
#ifdef OW_I18N
				  end_sym_wc, end_sym_len,
				  start_sym_wc, start_sym_len, field_flag,
#else
				  end_sym, end_sym_len,
...
#ifdef OW_I18N
    if (start_sym_wc)
        free((char *)start_sym_wc);
    if (end_sym_wc)
        free((char *)end_sym_wc);       
#endif    
