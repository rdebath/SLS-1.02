...
Pkg_private Time xv_sel_get_last_event_time();
...
Destroy_status  status;
{
    ...
    XDeleteContext( sel_owner->dpy, (Window) sel_owner->selection, propCtx );
    propCtx = 0;
    
    XDeleteContext( sel_owner->dpy, (Window) sel_owner->selection, targetCtx );
    targetCtx = 0;
...
Sel_owner_info *sel_owner;
{
	...
	if ( XGetSelectionOwner( owner->dpy, seln ) != owner->xid )  {
	    ...
	    xv_error( (Xv_opaque)owner, ERROR_STRING,
                      "Selection ownership failed", 0 );

	    XDeleteContext( owner->dpy, owner->xid, selCtx );
	    owner->own = FALSE;
...
Sel_owner_info      *seln;
{
    ...
    if ( XGetWindowProperty( seln->dpy, seln->req->requestor,
        ...
        xv_error( seln->public_self, ERROR_STRING, 
		 "XGetWindowProperty Failed", 0 );
...
int           multipleIndex;
{
     ...
     */
    length = MAX_SEL_BUFF_SIZE( selection->dpy );
     ...
     */
    if ((selection->req->bytelength  > MAX_SEL_BUFF_SIZE( selection->dpy )) ||
	selection->req->incr ) {
	XWindowAttributes  winAttr;    
	int   status;
...
Sel_owner_info  *seln;
{
    ...
    req = seln->req;
    size = ((req->bytelength - req->offset) > MAX_SEL_BUFF_SIZE(seln->dpy)) ?
              MAX_SEL_BUFF_SIZE(seln->dpy) : (req->bytelength - req->offset);
...
#ifdef SEL_DEBUG
    printf("SendIncr: win=%d  prop=%d  type=%d  length=%d  format=%d\n", 
       req->requestor, req->property, req->type,( size / (req->format>>3) ) , req->format );
#endif
	 ...
	 */
	length = MAX_SEL_BUFF_SIZE( seln->dpy );
...
Sel_owner_info  *sel;
{
    ...
    if ( XGetWindowProperty( sel->dpy, sel->req->requestor,
			    ...
			    False,(Atom)AnyPropertyType, &target, &format, 
			    &length, &bytesafter, &prop) != Success )
        xv_error( sel->public_self, ERROR_STRING, 
		 "XGetWindowProperty Failed", 0 );
...
#ifdef SEL_DEBUG
    printf("SendIncrMessage: win=%d  prop=%d  type=%d  length=%d  format=%d\n", 
       sel->req->requestor, sel->req->property,sel->atomList->incr ,1 ,32 );
#endif
...
int              flag;
{
...
#ifdef SEL_DEBUG
    printf("RegisterSelClient: falg=%d selection=%d  xid=%d own=%d time=%ul dpy=%x\n",
	   flag, owner->selection, owner->xid, owner->own, owner->time, owner->dpy );
#endif
	 ...
	 */
	if (( infoPtr->client->xid == owner->xid ) && ( flag == SEL_DELETE_CLIENT )) {
	    infoPtr->client = NULL;
	    return;
	}
#ifdef SEL_DEBUG
	...
	if ( (infoPtr->client->selection == owner->selection) && 
	    (infoPtr->client->xid != owner->xid ) &&
            (flag == SEL_ADD_CLIENT) ) {

#ifdef SEL_DEBUG
	    printf("RegisterSelClient: Matched....\n");
#endif
