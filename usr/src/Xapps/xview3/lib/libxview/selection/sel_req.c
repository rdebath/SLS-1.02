...
va_list	    valist;
{
	...
	if ( types != NULL )
	    XFree( (char *)types );
	types = (Atom *) xv_calloc( sel_req->nbr_types, sizeof( Atom  ) );
	for (i = 0; i < sel_req->nbr_types; i++)
	...
	typeNames = (char **) xv_malloc( sel_req->nbr_types * sizeof (char *) );
	for (i = 0; i < sel_req->nbr_types; i++)
...
int            blocking;
{
     ...
     */
     if ( XGetWindowProperty( reply->seln->dpy, reply->requestor,
         ...
         xv_error( selReq->public_self, ERROR_STRING, 
...
int            multipleIndex;
{
	...
	reply->data = (Xv_opaque) xv_malloc( sizeof( Time ) );
     ...
     */    
    reply->length = MAX_SEL_BUFF_SIZE( reply->seln->dpy );
...
Atom           retType;
{
    ...
    do 
    {
	size = ( (byteLength - offset) > MAX_SEL_BUFF_SIZE(reply->seln->dpy)) ?
	MAX_SEL_BUFF_SIZE(reply->seln->dpy) : (byteLength - offset);
...
int             blocking;
{
	...
	xv_error( selReq->public_self, 
		 ERROR_STRING, "XGetWindowProperty Failed",
		  0 );
	 xv_sel_handle_error( SEL_BAD_CONVERSION, selReq, reply, 
		     reply->seln->atomList->multiple );
	 return FALSE;
...
Time           time;
{
    ...
    *reply = replyInfo;
	 ...
	 */
	arg = SelectionNotify;
	if ( !xv_sel_block_for_event( dpy,&event,replyInfo->timeout, xv_sel_predicate, 
			    (char *) &arg )) {
...
#ifdef SEL_DEBUG
    printf("xv_sel_handle_property_notify: win=%d  atom=%d  state=%d\n", 
