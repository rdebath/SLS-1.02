#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_utl.c 20.83 91/09/14";
#endif
#endif
...
#ifdef OW_I18N
	    value_str = null_string_wc;
	if (!(str = (wchar_t *) panel_strsave_wc((wchar_t *) value_str)))
	    return (size);
	if (image_string_wc(dest))
	    free(image_string_wc(dest));
    ...
    int             color_index;
{
	...
	switch (image->im_type) {
	  case PIT_STRING:
	    ...
	    if (image_font(image))
#ifdef OW_I18N
		font_xid = (XFontSet) xv_get(image_font(image), FONT_SET_ID);
#else
