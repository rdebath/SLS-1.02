#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_mlinetxt.c 1.14 91/09/14";
#endif
#endif
    ...
    Attr_avlist avlist;
{
...
#ifdef OW_I18N
    dp->terminators = (wchar_t *) mbstowcsdup((u_char *) "\n\r\t");
#else
    dp->terminators = (char *) panel_strsave((u_char *) "\n\r\t");
#endif /* OW_I18N */
    dp->textsw = xv_create(dp->frame, TEXTSW,
    ...
    xv_set(xv_get(dp->textsw, OPENWIN_NTH_VIEW, 0),
    ...
    register Attr_avlist avlist;
{
	...
	switch (avlist[0]) {
	  ...
	  case PANEL_NOTIFY_STRING:
	    ...
	    dp->terminators = (wchar_t *) mbstowcsdup((u_char *) avlist[1]);
	    break;
	  ...
	  case PANEL_VALUE:
	    if (dp->value)
		xv_free(dp->value);
	    dp->value = (wchar_t *) mbstowcsdup((u_char *) avlist[1]);
    ...
    va_list valist;
{
	...
	switch (*(int *) valist) {
	  case 0:
	    return xv_get(dp->textsw, OPENWIN_NTH_VIEW, 0);
	  case 1:
...
#ifdef OW_I18N
      case PANEL_NOTIFY_STRING:
	if (dp->str_mbs) xv_free(dp->str_mbs);
	dp->str_mbs = (char *) wcstombsdup((wchar_t *)dp->terminators);
	return (Xv_opaque) dp->str_mbs;
	...
	return (Xv_opaque) dp->terminators;
      ...
      case PANEL_READ_ONLY:
	return xv_get(dp->textsw, TEXTSW_READ_ONLY);
	...
	if (length > dp->value_size) {
	    ...
	    dp->str_mbs = xv_malloc(length);
	    dp->value_size = length;
	}
	xv_get(dp->textsw, TEXTSW_CONTENTS, 0, dp->str_mbs, length-1);
	dp->str_mbs[length-1] = 0;  /* NULL terminate the string */
	return (Xv_opaque) dp->str_mbs;
    ...
    Event *event;
{
	...
	if (event_is_string(event)) {
	     ...
	     */
	    ie_string_wc = mbstowcsdup((char *) event_string(event));
	    p = ie_string_wc;
		...
		pos = wschr(dp->terminators, tmp_char_wc);
	    }
	    return event_is_down(event) && pos != 0;
	}
#else
    ...
    Mltxt_info	   *dp;
{
    ...
    win_translate_xy(PANEL_PUBLIC(panel), dp->frame,
