#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_gauge.c 1.24 91/09/14 Copyr 1984 Sun Micro";
#endif
#endif
    ...
    register Attr_avlist avlist;
{
	...
	switch (attr) {
	  case PANEL_VALUE:
	    ...
	    if (*avlist) {
		dp->min_tick_string = 
		    (wchar_t *) mbstowcsdup((char *) *avlist);
	    } else
    ...
    va_list         valist;
{
	...
	string_mbs = (char *)wcstombsdup((wchar_t *)dp->min_tick_string);
	...
	return (Xv_opaque) dp->min_value;
      ...
      case PANEL_MAX_TICK_STRING:
	if (string_mbs) xv_free(string_mbs);
	string_mbs = (char *)wcstombsdup((wchar_t *)dp->max_tick_string);
        return (Xv_opaque) string_mbs;
