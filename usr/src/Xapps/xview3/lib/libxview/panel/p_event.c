#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_event.c 20.49 91/09/20";
#endif
#endif
    ...
    Notify_event_type type;
{
	...
	if (panel->paint_window->view) {
	    /* Process events in canvas_paint_event */
	    return notify_next_event_func(paint_window, (Notify_event)event,
                                          arg, type);
    ...
    /* Process the event */
    result = panel_default_event(PANEL_PUBLIC(panel), event, arg);
    if (result != NOTIFY_DONE)
