#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_num_txt.c 20.40 91/09/14";
#endif
#endif
...
#ifdef OW_I18N
      case PANEL_NOTIFY_STRING:
        dp->terminators = wcstombsdup((wchar_t *)dp->terminators_wc);
...
#ifdef OW_I18N
    wsscanf((wchar_t *) xv_get(dp->text_field, PANEL_VALUE_WCS), "%d", &value);
#else
    sscanf((char *) xv_get(dp->text_field, PANEL_VALUE), "%d", &value);
#endif OW_I18N
    return value;
