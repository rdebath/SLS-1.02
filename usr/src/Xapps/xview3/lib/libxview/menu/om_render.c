#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)om_render.c 20.152 91/09/14";
#endif
#endif
...
unsigned short menu_gray75_data[16] = {	/* 75% gray */
    ...
    Xv_menu_item_info *parent;
{
    ...
    if (m->group_info->vinfo->visual == (Visual *)xv_get(group->client_window, XV_VISUAL))
    ...
    Event          *event;
{
	...
	if (event_is_string(event)) {
	    ...
	    newitem = m->curitem; /* could be 0 */
	    match_string = (wchar_t *) event_string(event);
	    do {
		newitem++;
		if (newitem > m->nitems) {
		    if (m->curitem == 0)
		...
		if (mi->title && m->pin)
		    ...
		    match_char = (wchar_t) mi->image.string[0];
    ...
    register Rect  *rect;
{
	...
	xv_error(0,
		 ERROR_STRING, 
		    XV_MSG("Menu too large for screen"),
		 ERROR_PKG, MENU,
		 0);
	return FALSE;
    }
    ...
    int            *item_top;	/* menu item top (output parameter) */
{
    ...
    if (m->column_major) { /* column major layout */


	if (item_nbr > m->nrows-1 && m->item_list[0]->title) {
	    item_nbr = item_nbr - m->nrows;
	    column = item_nbr / (m->nrows-1);
	    row = item_nbr % (m->nrows-1);
	    column++;
	    row++;
        } else {
	    column = item_nbr / m->nrows;
	    row = item_nbr % m->nrows;
	}
	    ...
	    if (item_nbr) {
	       row = ((item_nbr-1) / m->ncols) + 1;
	       column = (item_nbr-1) % m->ncols;
	    } else
	        row = column = 0;  /* title item */
	} else {
	    row = item_nbr / m->ncols;
	    column = item_nbr % m->ncols;
        }
...
#ifdef OW_I18N
	    x += (width - overall_logical_extents.width)/2;
