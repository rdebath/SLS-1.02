#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)om_public.c 20.132 91/09/14";
#endif
#endif
		...
		break;
              ...
              case MENU_STRING:{
                ...
                if( mi->image.string_mbs ) {
                    correct = (strcmp( mi->image.string_mbs , (char *)attrs[1]) == 0 );
                }
                else {
                    wcs_len = wslen( mi->image.string );
                    mi->image.string_mbs = (char *)wcstombsdup(mi->image.string);
                    correct = (strcmp( mi->image.string_mbs , (char *)attrs[1]) == 0 );
                }
                break;
              }
              case MENU_STRING_WCS:
                correct = mi->image.string && wscmp(mi->image.string,
                    (wchar_t *) attrs[1]) == 0;
                break;
#else
	      case MENU_STRING:
