#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)windowdrop.c 1.12 91/09/14";
#endif
#endif
    ...
    Xv_drop_site	 dropItem;
{
    ...
    /* If the top level window is not mapped, then don't update the
     * interest property, it will be done when the top level window is
     * mapped.
     */
    if (win->map)
        win_update_dnd_property(win);
