#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)sel_agent.c 1.70 91/09/14";
#endif
#endif
    ...
    Atom                target;
{
    ...
    XV_BCOPY(replier_data->context + agent->get_prop.offset, temp,
    ...
    Seln_request   *buffer;
{
    ...
    if (context->first_time) {
        ...
        if (context->chars_remaining > STRLEN((CHAR *)reply)) {
            ...
            if (context->buffer == NULL) {
                xv_error(NULL,
                    ERROR_STRING,
                        XV_MSG("selection: malloc() failed"),
                    0);
                return(SELN_FAILED);
            }   
        }   
    ...
    if (context->chars_remaining == 0) {
        ...
        if (context->buffer != NULL) {
            XV_BCOPY((char *)reply, (char *)(context->buffer + context->offset),
                        length * sizeof(CHAR));
