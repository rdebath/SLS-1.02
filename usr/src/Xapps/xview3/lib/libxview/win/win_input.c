#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)win_input.c 20.184 91/09/14";
#endif
#endif
    ...
    Xv_object      	*pwindow;
{
    ...
    switch (event_type) {
      ...
      case KeyRelease:{
	    ...
	    int             modifiers = 0, sem_action, keyboard_key;
	    int	  	    status = True,
...
#ifdef OW_I18N
		   /*
		    * This code has a problem when conversion mode is ON.
		    * Above transaction should be done in XIM. This is a
		    * temporary code.                        7/18/91
		    */
		   goto DoLookup;
#else
...
DoLookup:
			...
			buffer_backup = (char *)malloc(buf_length + 1);
			buf_length = XmbLookupString(ic, ek, buffer_backup,
    ...
    Event          *event;
{
	...
	break;
      case SERVER_WM_DRAGDROP_TRIGGER_TYPE: {
		...
		Attr_pkg	type = xv_get(event_window(event), WIN_TYPE);

		/* On V2 clients, only deliver new style drop events to
		 * panels and textsw since they are the ones who know
		 * how to handle them.
		 */
		if ((type != TERMSW_VIEW_TYPE) && (type != TERMSW_TYPE) &&
		    (type != TEXTSW_TYPE) && (type != PANEL_TYPE))  {
	    	    return(True);
		}
	    }
    ...
    Event		*event;
{
    ...
    switch(event_action(event)) {
      ...
      case ACTION_CUT:
	  if (key_type !=
			(Atom)xv_get(xv_screen(info), SCREEN_SELECTION_STATE)) {
              XChangeProperty(xv_display(info), (XID)xv_get(xv_root(info),
							    XV_XID),
