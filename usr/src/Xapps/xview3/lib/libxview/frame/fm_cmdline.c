...
static char     sccsid[] = "@(#)fm_cmdline.c 20.38 91/09/14";
#endif
    ...
    Frame           frame_public;
{
	...
	string = defaults_get_string("font.name",
				     "Font.Name", (char *)NULL);
	
	font = xv_find(frame_public, FONT, FONT_NAME, string, NULL);
    ...
    /*
     * Skip if font was specified
     */
    if (!font_found && defaults_exists("window.scale", "Window.Scale"))  {
	int		scale;
	int		scale_found = FALSE;

	string = defaults_get_string("window.scale",
				     "Window.Scale", (char *)NULL);
