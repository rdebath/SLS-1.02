#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)help.c 1.70 91/09/14";
#endif
#endif
...
Pkg_private FILE   *help_find_file();
Pkg_private int	    help_get_arg();
Pkg_private char   *help_get_text();
    ...
    Event          *client_event;
{
    ...
    if (help_get_arg(client_data, &more_help_cmd) == XV_OK)
	text = help_get_text();
    else
	text = NULL;
    if (!text) {
    ...
    xv_set(help_info->mglass_msg, PANEL_LABEL_IMAGE, help_info->help_image, 0);
    ...
    Event          *client_event;
{
	...
	file_ptr = help_find_file(help_string_filename);
	if (!file_ptr) {
	    free(seln_string);
	    help_request_failed(client_window, NULL, 
