#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)term_ntfy.c 20.51 91/09/14";
#endif
#endif
	...
	break;
      case KBD_USE:
        ...
        if (!ttysw_getopt(ttysw, TTYOPT_TEXT) &&
            xv_get(TTY_PUBLIC(ttysw), WIN_USE_IM))
    ...
    int             maybe_partial;
{
    ...
    register CHAR  *cp;
#ifdef OW_I18N
    int             length = (int) xv_get(textsw, TEXTSW_LENGTH_WC);
#else
...
#ifdef OW_I18N
    (void) xv_get(textsw, TEXTSW_CONTENTS_WCS, start_from, iwbp, cmd_length);
#else
...
#ifdef OW_I18N
    int             length = (Textsw_index) xv_get(textsw, TEXTSW_LENGTH_WC);
#else
...
#ifdef OW_I18N
        (void) xv_get(textsw, TEXTSW_CONTENTS_WCS,
#else
...
#define	BUFSIZE 1024
	    ...
	    Textsw_index    length =
#ifdef OW_I18N
            (int) xv_get(textsw, TEXTSW_LENGTH_WC);
#else
...
#ifdef OW_I18N
                length = (Textsw_index) xv_get(textsw, TEXTSW_LENGTH_WC);
#else
