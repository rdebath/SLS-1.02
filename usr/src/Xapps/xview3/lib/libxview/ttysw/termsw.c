#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)termsw.c 1.51 91/09/14";
#endif
#endif
...
Pkg_private void ttysw_interpose_on_textsw();
    ...
    Textsw_attribute avlist[];
{
	...
	Xv_opaque       parent_font = (Xv_opaque) xv_get(termsw_public, WIN_FONT);
    ...
    termsw_folio->enable_scroll_stay_on = FALSE;
...
#   endif /* XV_USE_SVR4_PTYS */
    ...
    *defaults++ = 0;
     ...
     */
    (void) ttysw_set_menu(termsw_public);
    ...
    Attr_avlist     avlist;
{
	...
	switch ((int) attrs[0]) {
	  case TTY_INPUT:{
		Ttysw_view_handle ttysw_view = TTY_VIEW_PRIVATE_FROM_TERMSW_VIEW
		(termsw_view_public);
		Termsw_folio    termsw_folio = TERMSW_FOLIO_FROM_TERMSW_VIEW(termsw_view_public);
		Ttysw_folio     ttysw = TTY_FOLIO_FROM_TTY_VIEW_HANDLE(ttysw_view);
    ...
    va_list         args;
{
    ...
    if ((attr == (Termsw_attribute) XV_IS_SUBTYPE_OF) && (va_arg(args, Xv_pkg *) == TEXTSW_VIEW))
	return ((Xv_opaque) TRUE);
    ...
    Destroy_status  status;
{
    ...
    *               termsw_view_object = (Xv_termsw_view *) termsw_view_public;
    ...
    Termsw_view_handle view = TERMSW_VIEW_PRIVATE(termsw_view_public);
    Termsw_folio    folio = TERMSW_FOLIO_FROM_TERMSW_VIEW(termsw_view_public);
    int             result = XV_OK;
    ...
    Xv_Window       termsw_view_public;
{
	 ...
	 */
#ifdef OW_I18N
        length = (int) xv_get(termsw_view_public, TEXTSW_LENGTH_WC);
#else
	length = (int) xv_get(termsw_view_public, TEXTSW_LENGTH);
#endif
	termsw_folio->user_mark =
	    textsw_add_mark(termsw_view_public, length, TEXTSW_MARK_DEFAULTS);
	...
	if (termsw_folio->append_only_log) {
		...
		textsw_add_mark(termsw_view_public,
...
termsw_menu_active()
{
    return (menu_currently_active == TRUE);
}

Pkg_private int
termsw_menu_inactive()
{
    return (menu_currently_active == FALSE);
}
