#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ttyselect.c 20.41 91/09/14";
#endif
#endif
    ...
    int             multi;
{
	...
	if (ttysel_insel(ttysel, &tspe) && timercmp(&td, &maxinterval, <)) {
	    (void) ttysel_adjust(ttysw, event, TRUE, TRUE);
	    return;
	}
    }
    if (ttysel->sel_made)
...
#else
	    proc(begin->tsp_col, length(image[row]), row, data, ttysel);
#endif
...
#else
	    proc(0, length(image[row]), row, data, ttysel);
#endif
	}
    }
}
    ...
    *te = *tb;
	...
	break;
      case SEL_WORD:{
	    ...
	    for (col = te->tsp_col; col < (int)length(line); col++) {
		chr = line[col];
		if ( delim_table[chr] != match_mode )
		    break;
	    }
    ...
    register int    start, finish, row, *count;
{
    ...
    *count += finish + 1 - start;
    if (length(image[row]) == finish && finish == ttysw_right) {
	*count -= 1;		/* no CR on wrapped lines	 */
    }
}
#endif
    ...
    register FILE  *file;
{
    ...
    (void) ttysw_reset_conditions(TTY_VIEW_HANDLE_FROM_TTY_FOLIO(ttysel_ttysw));
}
    ...
    register Ttysw_folio ttysw;
{
    ...
    if (ok = ttysw_is_seln_nonzero(ttysw, SELN_SHELF)) {
    ...
    register int    max_length;
{
    ...
    curr_row = ttysel->sel_begin.tsp_row;
    while (curr_row < last_row) {
	row_len = MIN((int)length(image[curr_row]) - curr_col, max_length);
	index = row_len;
	src = image[curr_row] + curr_col;
	while (index--) {
	    *dest++ = *src++;
	...
	*dest++ = *src++;
    ...
    if (end_col == length(image[curr_row]) && end_col < ttysw_right) {
	dest[-1] = '\n';
	*dest = '\0';
	max_length--;
    }
    ...
    register Seln_request *buffer;
{
   ...
   /*
    *  BUG! ; Ttysw is confused if null characters are padded at the
    ...
    */
    ...
    (void) ttysw_reset_conditions(TTY_VIEW_HANDLE_FROM_TTY_FOLIO(ttysw));
