#ident "@(#)ol_init.c	1.50 91/02/27 SMI"
...
/*
 * olgx_main_initialize() Mallocs a ginfos struct,initilaises it with the
 * respective values passed and returns a pointer to the ginfo struct
 */
...
#ifdef OW_I18N
/* FIX_ME_BIN: Binary/API compat issue */
		     textfontset, pixvals, stipple_pixmaps)
#else
...
#ifdef OW_I18N
				 glyphfont_struct, textfontset, pixvals,
#else
...
void
#ifdef OW_I18N
olgx_set_text_font(info, fontset_info, flag)
    Graphics_info  *info;
...
#endif
{
...
#ifdef OW_I18N
    if (olgx_cmp_fonts(fontset_info, info->textfontset))
	return;

    info->textfontset = fontset_info;
#else OW_I18N
	...
	if (info->gc_rec[OLGX_TEXTGC]->ref_count > 1) {
		...
		if (existing_gcrec != info->gc_rec[OLGX_TEXTGC]) {
			...
			existing_gcrec = olgx_gcrec_available(perdispl_res_ptr,
				   info->gc_rec[OLGX_TEXTGC_REV]->valuemask,
				    &info->gc_rec[OLGX_TEXTGC_REV]->values);
			existing_gcrec->ref_count += 1;
...
#ifdef OW_I18N
	info->gc_rec[OLGX_TEXTGC] = olgx_get_gcrec(perdispl_res_ptr,
		info->drawable[0], info->depth, valuemask & ~GCFont, &values);
#else
	values.font = info->textfont->fid;
	info->gc_rec[OLGX_TEXTGC] = olgx_get_gcrec(perdispl_res_ptr,
                                                   info->drawable[0],
