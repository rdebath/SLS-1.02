#ident "@(#)ol_sb.c	1.27 91/07/03 SMI"
...

{
	...
	if (state & OLGX_UPDATE) {
	    ...
	    XFillRectangle(ginfo->dpy, win, ginfo->three_d ?
                                              ginfo->gc_rec[OLGX_BG1]->gc :
                                              ginfo->gc_rec[OLGX_WHITE]->gc,
                          result_rect->x - 1, result_rect->y, 
                          result_rect->width + 1, result_rect->height + 1);
	...
	rectangles[0].width = (prop_length) ? (prop_pos - rectangles[0].x)
	    : (elev_pos - rectangles[0].x - 1);
	rectangles[1].width = (x + length - cable_offset - 2)
	    - rectangles[1].x;
	...
	if (prop_length) {
	   ...
	   rectangles[1].width = (prop_pos + prop_length) - (elev_pos + sbheight - 1);
	...
	rectangles[0].height = (prop_length) ? (prop_pos - rectangles[0].y)
	    : (elev_pos - rectangles[0].y - 1);
	rectangles[1].height = (y + length - cable_offset - 2)
	    - rectangles[1].y;
	...
	if (prop_length) {
	    ...
	    rectangles[1].height = (prop_pos + prop_length) - 
                                   (elev_pos + sbheight - 1);

	    if (rectangles[1].height < 0)
		rectangles[1].height = 0;
