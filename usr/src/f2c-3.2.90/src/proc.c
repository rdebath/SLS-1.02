...
zap_changes()
{
	...
	for(cp = newprocs; cp; cp = cp->nextp)
...
struct Entrypoint *ep;
{
	...
	Addrp mkarg(), rs;
	int it, k;
	struct Extsym *entryname = ep->entryname;

	if (++nentry > 1)
		p1put_label((long)(extsymtab - entryname - 1));
