...
struct Addrblock *addrp;
{
    ...
    new_vars = mkchain((char *)cpexpr(addrp), new_vars);
} /* declare_new_addr */


write_nv_ident_help (outfile, addrp)
FILE *outfile;
struct Addrblock *addrp;
{
    int eltcount = 0;
...
struct Literal *litp;
{
    ...
    switch (litp -> littype) {
		...
		if (val < 0)
		    sprintf (buf, "cs_n%d", -val);
		else
		    sprintf (buf, "cs__%d", val);
	    } else
	...
	case TYLONG:
		...
		if (val < 0)
		    sprintf (buf, "c_n%d", -val);
		else
		    sprintf (buf, "c__%d", val);
	    } else
		sprintf (buf, "c_b%d", litp -> litnum);
	    break;
	case TYLOGICAL:
...
/* write_globals -- after every function has been translated, we need to
   output the global declarations, such as the static table of constant
   values */

write_globals (outfile)
FILE *outfile;
{
...
/* write_common_decls -- outputs the common declarations in one of three
...
/* Construct the proper, condensed list of structs; eliminate duplicates
   from the initial list   ext -> allextp   */
		...
		write_out_struct (outfile, (chainp) comm -> datap);
		...
		define_start(outfile, ext->cextname,
			comm_union_name(count), "");
		switch (which) {
		    case ONE_STRUCT:
		        output_extern (outfile, ext);
		        break;
		    case UNION_STRUCT:
		        nice_printf (outfile, "(");
			output_extern (outfile, ext);
...
write_out_struct (outfile, var_list)
FILE *outfile;
chainp var_list;
{
    int last_type = -1;
	...
	int type;
	char *comment = NULL, *write_array_decls ();

	if (var == (Namep) NULL)
	    err ("write_out_struct:  null variable");
	else if (var -> tag != TNAME)
	    erri ("write_out_struct:  bad tag on variable '%d'",
		    var -> tag);
...
/* Character type is really a string type.  Put out a '*' for parameters
   with unknown length and functions returning character */
	...
	var -> vstg = STGAUTO;
	output_name (outfile, var);
	if (var -> vclass == CLPROC)
	    nice_printf (outfile, "()");
	else if (var -> vdim)
	    comment = write_array_decls(outfile, var->vdim,
				var->vtype == TYCHAR && ISICON(var->vleng)
				? (int)var->vleng->constblock.Const.ci : 1);
	...
	for(ext = extsymtab; ext < nextext; ext++)
		if (ext->extstg == STGCOMMON && !ext->extinit) {
			...
			for(c = 1; comm; comm = comm->nextp) {
				...
				next_tab(of);
				write_out_struct(of, (chainp)comm->datap);
				prev_tab(of);
				if (Union)
					nice_printf(of, "} _%d;\n", c++);
				}
