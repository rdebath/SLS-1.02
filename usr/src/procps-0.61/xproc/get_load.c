     ...
     caddr_t	call_data;	/* pointer to (double) return value */
{
	...
	if (!(f = fopen ("/proc/meminfo", "r"))) {
		perror("xmem : cannot open /proc/meminfo ");
		exit(1);
	}
	if (fgets(buf, sizeof(buf), f) != buf ||
			fgets(buf, sizeof(buf), f) != buf ||
			sscanf(buf, "%*s %lf %*d %lf", &value, &avail) != 2 ||
			fgets(buf, sizeof(buf), f) != buf ||
			sscanf(buf, "%*s %d %*d %d", &i, &j) !=  2) {
		perror("xmem : /proc/meminfo is in a bad format ");
		exit(1);
	}

	*(double *)call_data = 10.0 * (avail + (double )j) /
		(value + (double )i);
#else
