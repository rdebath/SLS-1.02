...
#ifdef MEMORY
    toplevel = XtAppInitialize(&app_con, "XMem", options, XtNumber(options),
...
#ifdef MEMORY
	(void) sprintf (name, "%s.paned.mem.update", XtName(toplevel));
	found = XrmGetResource (XtScreenDatabase(XtScreen(toplevel)),
				name, "XMem.Paned.StripChart.Interval",
				&type, &db_value);
#else
...
#ifdef MEMORY
    	load = XtCreateManagedWidget ("mem", stripChartWidgetClass,
