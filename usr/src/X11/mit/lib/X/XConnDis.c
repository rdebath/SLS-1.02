/*
 * $Header: /home/x_cvs/mit/lib/X/XConnDis.c,v 1.5 1992/08/25 08:57:30 dawes Exp $
    ...
    char **saddrp;			/* RETURN */
{
...
#ifndef SVR4 /* SVR4 has just STREAMS pttys */

  struct strfdinsert buf;
  struct strbuf ctlbuf;

  /*
   * ISC's local connection 
   */
  ...
  if ((server = open (server_path, O_RDWR)) >= 0) 
	...
	if (ret >= 0) {
	  /* The msg we got via getmsg is the result of an I_FDINSERT, so if
	   * we do a putmsg with whatever we recieved, we're doing another
	   * I_FDINSERT ...
	   */
