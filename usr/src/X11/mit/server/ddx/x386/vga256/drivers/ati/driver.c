 ...
 * desired.
 * 
 * Revised: Fri Aug 21 14:03:55 1992 by root
 *
 */
 ...
 *
 * $Header: /home/x_cvs/mit/server/ddx/x386/vga256/drivers/ati/driver.c,v 1.5 1992/09/26 17:42:01 dawes Exp $
 */
 ...
 *
 * I wrote the ATIProbe and ATIEnterLeave routines from scratch.
 *
 * I also re-organized the ATISave and ATIRestore routines.  This was necessary,
 * since the vgaHWSave routine did not save the DAC values.  There also appeared
 * to be a timing problem associated with the 486-33, but this is not well
...
vgaATIPtr restore;
{
	...
	for (i=0; i<25; i++) outw(vgaIOBase + 4,(restore->std.CRTC[i] << 8) | i);
...
vgaATIPtr save;
{
	...
	for (i=0; i<9;  i++) { outb(0x3CE,i); save->std.Graphics[i]  = inb(0x3CF); }

	for (i=0; i<5;  i++) { outb(0x3C4,i); save->std.Sequencer[i] = inb(0x3C5); }
...
DisplayModePtr mode;
{
	...
	static unsigned char extregPlusXLAndOrMasks[][3] = {
		{ ER_B0, 0xd9, 0x28 },	/* Experiment: 28 was 20 */
		{ ER_B1, 0x87, 0x00 },
		{ ER_B3, 0xaf, 0x00 },
		{ ER_B5, 0x7f, 0x00 },
		{ ER_B6, 0xe2, 0x04 },
	...
	vgaHWInit(mode, sizeof(vgaATIRec));
	...
	if ((ATIBoard > ATI_BOARD_V3) && (mode->Flags & V_INTERLACE))
		new->ATIRegE |= 0x02;		/* FIXME: untested */
...
ATIAdjust()
{
	...
	Base = (vga256InfoRec.frameY0 * vga256InfoRec.virtualX + vga256InfoRec.frameX0) >> 3;
...
ATIProbe()
{
   ...
   if (vga256InfoRec.chipset) {
      if (strcmp( vga256InfoRec.chipset, ATIIdent()))
	    return FALSE;
   } else {
#ifdef MACH386
#define C 0xc0000
...
#define KERN_SUCCESS 0
      if (KERN_SUCCESS != vm_allocate(task_self(), &screen_addr, S, TRUE)) {
      ...
      bcopy(screen_addr+0x31, bios_signature, signature_length);
      if (KERN_SUCCESS != vm_deallocate(task_self(), screen_addr, S)) {
...
#undef S
	  ...
	  || lseek( fd, 0xc0031, SEEK_SET) < 0
	  || read( fd, bios_signature, signature_length ) != signature_length) {

	 if (fd >= 0) close( fd );
	 ErrorF( "Failed to read VGA BIOS.  Cannot detect ATI card.\n" );
	 return FALSE;
      }
      if (strncmp( signature, bios_signature, signature_length )) {
	 TRACE(("ATIProbe: Incorrect Signature\n"));
   ...
   /* Assume an ATI card. */
   vga256InfoRec.chipset = ATIIdent();
...
#define KERN_SUCCESS 0
   ...
   bcopy(screen_addr, bios_data, bios_data_size);
   if (KERN_SUCCESS != vm_deallocate(task_self(), screen_addr, S)) {
      ErrorF("Failed vmdeallocate %x\n", S);
      close(fd);
      return FALSE;
...
#undef S
   ...
   if ((fd = open( "/dev/mem", O_RDONLY )) < 0
      ...
      if (fd >= 0) close( fd );
      ErrorF( "Failed to read VGA BIOS.  Cannot set up ATI card.\n" );
      return FALSE;
   }
   close( fd );
#endif /* MACH386 */
   ...
   if (bios_data[ 0x43 ] < 5) {
      ErrorF( "  WARNING: This ATI driver may not work correctly with your board!\n" );
   }
   ErrorF( "\n" );
...
#else
      ioctl( x386Info.consoleFd, KDADDIO, ATIExtReg );
      ioctl( x386Info.consoleFd, KDADDIO, ATIExtReg+1 );
#endif
#endif
    }
  else
...
#ifdef linux
      ioperm( ATIExtReg, 2, 0 );
#else
      ioctl( x386Info.consoleFd, KDDELIO, ATIExtReg );
      ioctl( x386Info.consoleFd, KDDELIO, ATIExtReg+1 );
#endif
#endif
    }
}
