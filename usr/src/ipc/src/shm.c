...
static int max_shmid = 0; /* every valid id is <= max_shmid */
static struct wait_queue *shm_lock = NULL;
static struct shmid_ds *shm_segs[SHMMNI];
static ulong swap_attempts=0, swap_successes = 0, used_segs = 0;
...
int sys_shmctl (int shmid, int cmd, struct shmid_ds *buf)
{
	...
	case IPC_INFO: 
	{
		...
		verify_area (VERIFY_WRITE, buf, sizeof (struct shminfo));
	...
	{ 
		...
		if (!buf)
			return -EFAULT;
		verify_area (VERIFY_WRITE, buf, sizeof (shm_info));
	...
	case SHM_STAT:
		if (!buf)
			return -EFAULT;
		verify_area (VERIFY_WRITE, buf, sizeof (*shp));
		if (shmid > max_shmid)
			return -EINVAL;
		shp = shm_segs[shmid];
