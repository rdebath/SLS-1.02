    ...
    { var reg2 ffloat x_ = ffloat_value(x);
      ...
      if (uexp <= FF_exp_mid) # e<=0 ?
        ...
        { if (uexp > FF_exp_mid+FF_mant_len) # e > 23 ?
            ...
            { var reg1 uint32 mask = # Bitmaske: Bits 23-e..0 gesetzt, alle anderen gelöscht
                bit(FF_mant_len+1+FF_exp_mid-uexp)-1;
      ...
      if (uexp < FF_exp_mid) # x = 0.0 oder e<0 ?
        ...
        { if (uexp > FF_exp_mid+FF_mant_len) # e > 23 ?
            ...
            if (uexp > FF_exp_mid+1) # e>1 ?
              { var reg4 uint32 bitmask = # Bitmaske: Bit 23-e gesetzt, alle anderen gelöscht
                  bit(FF_mant_len+FF_exp_mid-uexp);
                var reg3 uint32 mask = # Bitmaske: Bits 22-e..0 gesetzt, alle anderen gelöscht
                  bitmask-1;
