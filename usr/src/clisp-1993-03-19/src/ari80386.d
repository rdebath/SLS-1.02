...
msld1:    movl    (%edi,%ecx,4),%eax # *ptr
          ...
          decl    %ecx               # count--, ptr--
          jnz     msld1
msld2:  movl    %ebp,%eax       # Ergebnis := letzter Übertrag
        popl    %ebx            # %ebx zurück
        popl    %edi            # %edi zurück
        popl    %ebp            # %ebp zurück
        ret
...
C(divu_loop_up:)
        ...
        jecxz   dlu2            # %ecx = 0 ?
dlu1:     movl    (%edi),%eax     # nächstes Digit *ptr
...
C(divucopy_loop_up:)
        ...
        jecxz   dclu2           # %ecx = 0 ?
        subl    %edi,%esi
dclu1:    movl    (%esi,%edi),%eax # nächstes Digit *ptr
          divl    %ebx            # Division von %edx|%eax durch %ebx
          movl    %eax,(%edi)     # Quotient %eax ablegen, Rest in %edx behalten
          leal    4(%edi),%edi    # sourceptr++, destptr++
          decl    %ecx
