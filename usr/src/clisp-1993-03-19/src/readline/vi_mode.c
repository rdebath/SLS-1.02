/* vi_mode.c -- changed by Bruno Haible, 16 March 1993 */
...
rl_vi_movement_mode ()
{
  forward void vi_done_inserting RL((void));
     ...
     int key, *nextkey;
{
  ...
  forward void rl_digit_loop1 RL((void));
...
rl_vi_first_print ()
{
  forward void rl_back_to_indent RL((int ignore1, int ignore2));
  ...
  if (!vi_replace_map)
    {
      ...
      for (i = ' '; i < NUMCHARS; i++)
	if (i != 127)
	  vi_replace_map[i].function = (Function *)rl_vi_overstrike;

      vi_replace_map[RUBOUT].function = (Function *)rl_vi_overstrike_delete;
      vi_replace_map[ESC].function = (Function *)rl_vi_movement_mode;
      vi_replace_map[RETURN].function = (Function *)rl_newline;
      vi_replace_map[NEWLINE].function = (Function *)rl_newline;

      /* If the normal vi insertion keymap has ^H bound to erase, do the
         same here.  Probably should remove the assignment to RUBOUT up
         there, but I don't think it will make a difference in real life. */
      if (vi_insertion_keymap[CTRL ('H')].type == ISFUNC &&
	  vi_insertion_keymap[CTRL ('H')].function == (Function *)rl_rubout)
	vi_replace_map[CTRL ('H')].function = (Function *)rl_vi_overstrike_delete;
