/* readline.c -- changed by Bruno Haible, 19 March 1993 */
...
extern int tcsetattr RL((int fd, int optional_actions, struct termios * termios_p));
...
unsigned char *rl_line_buffer = (unsigned char *)NULL;
...
forward char* readline_internal RL((void));

/* Section "Signal Handling" */
forward void rl_set_signals RL((void));
forward void rl_clear_signals RL((void));
...
forward void rl_initialize RL((void));
forward void readline_initialize_everything RL((void));
forward void readline_default_bindings RL((void));

/* Section "Numeric Arguments" */
extern void rl_digit_argument RL((int ignore, int key));
forward void rl_discard_argument RL((void));
forward void rl_init_argument RL((void));
...
forward void rl_on_new_line RL((void));
forward void rl_on_new_line_with_prompt RL((void));
forward void rl_forced_update_display RL((void));
static void move_cursor_relative RL((int new, char* data));
forward void move_vert RL((int to));
extern void rl_message RL((char* string, int arg1, int arg2));
extern void rl_clear_message RL((void));

/* Section "Terminal and Termcap" */
forward void rl_reset_terminal RL((char *terminal_name));
forward void init_terminal_io RL((char *terminal_name));
static void output_character_function RL((int c));
static void output_some_chars RL((char* string, int count));
static void delete_chars RL((int count));
static void insert_some_chars RL((char* string, int count));
forward void backspace RL((int count));
forward void crlf RL((void));
forward void clear_to_eol RL((int count));
...
forward void rl_clean_up_for_exit RL((void));
...
forward void rl_change_case RL((int count, int op));
...
forward void rl_stop_output RL((int count, int key));
...
forward void rl_add_undo RL((enum undo_code what, int start, int end, char* text));
forward void free_undo_list RL((void));
...
forward void start_using_history RL((void));
forward void free_history_entry RL((HIST_ENTRY* entry));
forward void maybe_replace_line RL((void));
...
forward int rl_translate_keyseq RL((unsigned char * seq, unsigned char * array, int* len));
extern void rl_re_read_init_file RL((int count, int ignore));
forward int rl_read_init_file RL((char* filename));

/* Section "Parser Directives" */
forward void rl_parse_and_bind RL((char* string));
forward void rl_variable_bind RL((char* name, char* value));
forward int glean_key_from_name RL((char* name));

/* Section "Key Binding and Function Information" */
forward void rl_list_funmap_names RL((int ignore));
forward char** rl_invoking_keyseqs RL((Function* function));
extern int rl_dump_functions RL((int count));
forward void rl_function_dumper RL((int print_readably));

/* Section "String Utility Functions" */
#define strnicmp rl_strnicmp  /* avoid prototype clash in case the system already has strnicmp() */
#define stricmp rl_stricmp  /* avoid prototype clash in case the system already has stricmp() */
...
static int stricmp RL((char* string1, char* string2)); /* Caseless strcmp (). */
     ...
     Keymap map;
{
  ...
  if (key >= NUMCHARS && key < 2*NUMCHARS)
    {
...
readline_default_bindings ()
{
  ...
  if (ioctl (tty, TCGETA, &ttybuff) != -1)
	...
	keymap[(unsigned char)erase].function = (Function *)rl_rubout;

      if (kill != _POSIX_VDISABLE &&
	  keymap[(unsigned char)kill].type == ISFUNC)
	keymap[(unsigned char)kill].function = (Function *)rl_unix_line_discard;
	...
	if (nextc != _POSIX_VDISABLE &&
	  ...
	  keymap[(unsigned char)nextc].function = (Function *)rl_quoted_insert;
      }
	...
	if (werase != _POSIX_VDISABLE &&
	  ...
	  keymap[(unsigned char)werase].function = (Function *)rl_unix_word_rubout;
      }
#endif /* VWERASE */
    }
  ...
  while (1)
    {
	  ...
	  keymap[c].function == (Function *)rl_universal_argument)
	{
	  rl_numeric_arg *= 4;
	  continue;
	}
...
rl_on_new_line_with_prompt ()
{
  ...
  for (i=strlen(rl_prompt)+1; i<line_size; i++)
    { visible_line[i] = 0; invisible_line[i] = 1; }
  last_c_pos = strlen(rl_prompt);
  last_v_pos = 0;
  vis_botlin = last_lmargin = 0;
}
  ...
  if (any_typein)
    {
      ...
      while ((t = rl_get_char (&key)) &&
	     (keymap[key].type == ISFUNC &&
	      keymap[key].function == (Function *)rl_insert))
	string[i++] = key;
...
{
  if (rl_last_func == (Function *)rl_complete && !completion_changed_buffer)
     ...
     int what_to_do;
{
  ...
  unsigned char **completion_matches (), **matches;
  Function *our_func;
  ...
  if (rl_point)
      ...
      if (rl_completer_quote_characters)
	{
	  ...
	  for (scan = 0; scan < end; scan++)
	      ...
	      if (quote_char != '\0')
		{
		  ...
		  if (the_line[scan] == (unsigned char)quote_char)
		    {
      ...
      if (rindex (rl_completer_word_break_characters, the_line[rl_point]))
	{
	  ...
	  if (rindex (rl_basic_quote_characters, the_line[rl_point]) && (end - rl_point) > 1)
  ...
  if (rl_attempted_completion_function)
    {
      matches =
	(unsigned char **)(*rl_attempted_completion_function) (text, start, end);
 ...
 after_usual_completion:
  ...
  if (!matches)
      ...
      if (rl_ignore_completion_duplicates)
	{
	  ...
	  for (i = 0; matches[i]; i++);
	  if (i) qsort (matches+1, i-1, sizeof (char *), compare_strings);
	     ...
	     Copy all the non-dead entries into a new array. */
	  {
	    unsigned char **temp_array =
	      (unsigned char **)malloc ((3 + newlen) * sizeof (unsigned char *));

	    for (i = 1, j = 1; matches[i]; i++)
	      {
		if (matches[i] != (unsigned char *)-1)
		  temp_array[j++] = matches[i];
	      }

	    temp_array[j] = (unsigned char *)NULL;

	    if (matches[0] != (unsigned char *)-1)
	      free (matches[0]);
	...
	if (keymap[c].type == ISFUNC)
	  {
	    ...
	    if (f == (Function *)rl_reverse_search_history)
	      c = reverse ? -1 : -2;
	    else if (f == (Function *)rl_forward_search_history)
	      c =  !reverse ? -1 : -2;
	  }
      }
     ...
     char *(*entry_function) ();
{
  ...
  unsigned char **match_list =
    (unsigned char **)xmalloc (((match_list_size = 10) + 1) * sizeof (unsigned char *));
  ...
  while ((string = (*entry_function) (text, matches)) != NULL)
	...
	match_list = (unsigned char **)xrealloc
	  (match_list, ((match_list_size += 10) + 1) * sizeof (unsigned char *));
...
static struct {
  ...
  { "if", (Function *)parser_if },
  { "endif", (Function *)parser_endif },
  { "else", (Function *)parser_else },
     ...
     char *string;
{
  ...
  /* Ugly, but working hack to keep prefix-meta around. */
  else if (stricmp (funname, "prefix-meta") == 0)
    {
      ...
      seq[0] = key;
      seq[1] = '\0';
      rl_generic_bind (ISKMAP, seq, emacs_meta_keymap, keymap);
    }
