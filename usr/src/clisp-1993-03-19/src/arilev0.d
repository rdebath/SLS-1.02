...
#   MULU D1,D0
      ...
      #define mulu16(arg1,arg2)  \
        ({ var register uint16 _hi;                                        \
           ...
           __asm__("mulw %2"                                               \
                   : "=d" /* %dx */ (_hi), "=a" /* %ax */ (_lo)            \
                   : "g" ((uint16)(arg1)), "1" /* %eax */ ((uint16)(arg2)) \
                  );                                                       \
           highlow32(_hi,_lo);                                             \
         })
    #endif
...
# < 2^32*hi+lo : eine 48-Bit-Zahl
    ...
    #define mulu24(x,y,hi_zuweisung,lo_zuweisung)  \
      { var reg1 uint32 _x = (x);                                                       \
        var reg2 uint32 _y = (y);                                                       \
    ...
    #define mulu24(x,y,hi_zuweisung,lo_zuweisung)  \
      { var reg1 uint32 _x = (x);                                  \
        var reg2 uint32 _y = (y);                                  \
        var reg4 uint32 _erg21; # Teilresultat für Words 2 und 1 des Ergebnisses \
        var reg3 uint32 _erg10; # Teilresultat für Words 1 und 0 des Ergebnisses \
        { var reg3 uint16 _x1 = high16(_x);                        \
          var reg4 uint16 _y1 = high16(_y);                        \
...
# < 2^32*hi+lo : eine 64-Bit-Zahl
  extern uint32 mulu32_ (uint32 arg1, uint32 arg2); # -> Low-Teil
...
#   MULU.L D1,D1:D0
        ...
        #define mulu32(x,y,hi_zuweisung,lo_zuweisung)  \
          ({ var reg4 uint32 _x = (x);                                      \
             var reg4 uint32 _y = (y);                                      \
             var reg5 uint16 _x1 = high16(_x);                              \
             var reg5 uint16 _x0 = low16(_x);                               \
             var reg5 uint16 _y1 = high16(_y);                              \
             var reg5 uint16 _y0 = low16(_y);                               \
             var reg3 uint32 _hi = mulu16(_x1,_y1); # obere Portion         \
             var reg2 uint32 _lo = mulu16(_x0,_y0); # untere Portion        \
             {var reg1 uint32 _mid = mulu16(_x0,_y1); # 1. mittlere Portion \
              _hi += high16(_mid); _mid = highlow32_0(low16(_mid));         \
              _lo += _mid; if (_lo < _mid) { _hi += 1; } # 64-Bit-Addition  \
             }                                                              \
             {var reg1 uint32 _mid = mulu16(_x1,_y0); # 2. mittlere Portion \
              _hi += high16(_mid); _mid = highlow32_0(low16(_mid));         \
              _lo += _mid; if (_lo < _mid) { _hi += 1; } # 64-Bit-Addition  \
             }                                                              \
             hi_zuweisung _hi;                                              \
    ...
    #define mulu32(x,y,hi_zuweisung,lo_zuweisung)  \
    ...
    #if defined(MC680X0) || defined(SPARC) || defined(I80Z86) || defined(MIPS) || defined(HPPA) || defined(VAX)
      ...
      global uint32 mulu32_(x,y)
        var reg4 uint32 x;
        var reg4 uint32 y;
        { var reg5 uint16 x1 = high16(x);
          var reg5 uint16 x0 = low16(x);
          var reg5 uint16 y1 = high16(y);
          var reg5 uint16 y0 = low16(y);
          var reg3 uint32 hi = mulu16(x1,y1); # obere Portion
          var reg2 uint32 lo = mulu16(x0,y0); # untere Portion
          {var reg1 uint32 mid = mulu16(x0,y1); # 1. mittlere Portion
           hi += high16(mid); _mid = highlow32_0(low16(mid));
           lo += mid; if (lo < mid) { hi += 1; } # 64-Bit-Addition
          }
          {var reg1 uint32 mid = mulu16(x1,y0); # 2. mittlere Portion
...
# Es wird vorausgesetzt, daß arg1*arg2 < 2^32.
    ...
    #endif
  #elif defined(SPARC)
    extern uint32 mulu32_unchecked (uint32 x, uint32 y); # extern in Assembler
  #else
...
# < x = q*y+r
  #define divu_1616_1616(x,y,q_zuweisung,r_zuweisung)  \
    { var reg1 uint16 __x = (x);   \
      var reg2 uint16 __y = (y);   \
...
# < uint16 r: x mod y
# < x = q*y+r
  extern uint16 divu_3216_1616_ (uint32 x, uint16 y); # -> Quotient q
...
#   SWAP D1
      ...
      #define divu_3216_1616(x,y,q_zuweisung,r_zuweisung)  \
        ({ var reg1 uint32 __qr = divu_3216_1616_(x,y); # extern in Assembler \
      ...
      global uint16 divu_3216_1616_(x,y)
        var reg3 uint32 x;
        var reg2 uint16 y;
        { var reg1 uint16 q = floor(x,(uint32)y);
...
# < uint16 r: x mod y
# < x = q*y+r
  extern uint32 divu_3216_3216_ (uint32 x, uint16 y); # -> Quotient q
  extern uint16 divu_16_rest;                         # -> Rest r
...
#   SWAP D0        ; beide Quotienten kombinieren, liefert q
  #if defined(SPARC) || defined(I80Z86)
...
# < uint32 r: x mod y
# < x = q*y+r
  extern uint32 divu_3232_3232_ (uint32 x, uint32 y); # -> Quotient q
  extern uint32 divu_32_rest;                         # -> Rest r
  #if defined(SPARC) || defined(I80Z86) || defined(HPPA_DIV_WORKS)
    #define divu_3232_3232(x,y,q_zuweisung,r_zuweisung)  \
      divu_6432_3232(0,x,y,q_zuweisung,_EMA_ r_zuweisung)
...
# < x = q*y+r
  ...
  extern uint32 divu_32_rest;                                       # -> Rest r
      ...
      #define divu_6432_3232_(xhi,xlo,y) \
        ({var reg1 uint32 ___q; divu_6432_3232(xhi,xlo,y,___q=,); ___q; })
    #elif defined(SPARC)
      #define divu_6432_3232(xhi,xlo,y,q_zuweisung,r_zuweisung)  \
        ({ var reg1 uint32 _q = divu_6432_3232_(xhi,xlo,y); # extern in Assembler \
      ...
      #define divu_6432_3232_(xhi,xlo,y) \
        ({var reg1 uint32 ___q; divu_6432_3232(xhi,xlo,y,___q=,); ___q; })
    #elif defined(HAVE_LONGLONG) && !defined(HPPA_DIV_WORKS)
      #define divu_6432_3232(xhi,xlo,y,q_zuweisung,r_zuweisung) \
        ({var reg1 uint32 __xhi = (xhi);                           \
          var reg1 uint32 __xlo = (xlo);                           \
          var reg1 uint64 __x = (uint64)__xhi<<32 | (uint64)__xlo; \
          var reg1 uint32 __y = (y);                               \
          var reg1 uint32 __q = floor(__x,(uint64)__y);            \
    ...
    #if defined(MC680Y0) || defined(SPARC) || defined(I80Z86) || defined(HPPA)
      ...
      global uint32 divu_6432_3232_(xhi,xlo,y)
        ...
        { if (y <= (uint32)(bit(16)-1))
         ...
         {# y shiften:
          ...
          # incrementieren, um den Quotienten  q = floor(x/y)  und den Rest
          # x-floor(x/y)*y  der Division zu bekommen.
          { var reg2 uint16 y1_1 = high16(y)+1; # y1+1
            var reg7 uint16 q1;
            var reg8 uint16 q0;
            var reg3 uint32 r;
...
# < boolean sqrtp: /=0, falls x=y^2
  ...
  #define isqrt_32_16(x,y_zuweisung,sqrtp_zuweisung)  \
    { var reg4 uint32 _x = (x);                                      \
      var reg3 uint16 _x1 = high16(_x);                              \
      var reg1 uint16 _y = floor(_x1,2) | bit(16-1);                 \
      loop                                                           \
        { var reg2 uint16 _z;                                        \
          var reg5 uint16 _r;                                        \
          ...
          if (_z >= _y)                                              \
            { sqrtp_zuweisung (_z == _y) && (_r == 0); break; }      \
...
# < boolean sqrtp: /=0, falls x=y^2
    ...
    #define isqrt_64_32(xhi,xlo,y_zuweisung,sqrtp_zuweisung)  \
      { var reg3 uint32 _xhi = (xhi);                                   \
        var reg4 uint32 _xlo = (xlo);                                   \
        var reg1 uint32 _y = floor(_xhi,2) | bit(32-1);                 \
        loop                                                            \
          { var reg2 uint32 _z;                                         \
            var reg5 uint32 _rest;                                      \
    ...
    #define isqrt_64_32(xhi,xlo,y_zuweisung,sqrtp_zuweisung)  \
      { var reg4 uint32 _xhi = (xhi);                                        \
        var reg3 uint32 _xlo = (xlo);                                        \
        var reg6 uint16 _yhi;                                                \
        var reg5 uint16 _ylo;                                                \
       ...
       {var reg1 uint32 _z = (_xhi << 15) | (_xlo >> 17); # < 2^15*(2*yhi+1) \
        var reg2 uint32 _r = highlow32_0(_yhi);                              \
        if (_z >= _r)                                                        \
