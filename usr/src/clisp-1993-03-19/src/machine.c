...
/* Bruno Haible 10.9.1991, 12.10.1992, 6.12.1992 */
...
#define get_alignment(type,typestr)  \
  { printf("/* Type %s has sizeof = %ld and alignment = %ld. */\n",typestr,(long)sizeof(type),(long)alignmentof(type)); \
    if (!(typestr[0] == 'u') && !(typestr[strlen(typestr)-1] == '*'))                                                   \
...
long get_stack_direction()
  { auto char dummy;
    static char* dummyaddr = (char*)0;
    if (!(dummyaddr == (char*)0))
      { return (&dummy) - dummyaddr; }
    else
      { dummyaddr = &dummy; return get_stack_direction(); }
  }
