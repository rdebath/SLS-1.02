...
main()
{
  ...
  fprintf (stderr, "IPC_RMID=%d\t IPC_SET=%d\t IPC_STAT=%d\t SHM_LOCK=%d\t SHM_UNLOCK=%d\n",
	IPC_RMID, IPC_SET, IPC_STAT, SHM_LOCK, SHM_UNLOCK);
...
static void do_shmctl (int shmid, int cmd)
{
  ...
  fprintf (stderr, "shmctl: calling shmctl (%d, %d, buffer)\n",
	shmid, cmd);
  if (cmd == IPC_SET) 
    fprintf (stderr, "uid=%d\t gid=%d\t  perms = \n", ipcp->uid, ipcp->gid, 
	     ipcp->mode & 0777);
