...
int TileColour[] = {
	COLOR_WHITE,	/* Length 3 */
	COLOR_MAGENTA,	/* Length 5 */
	COLOR_RED,		/* Length 7 */
	COLOR_BLUE,		/* Length 9 */
	COLOR_GREEN,	/* Length 11 */
	COLOR_YELLOW, 	/* Length 13 */
	COLOR_CYAN,  	/* Length 15 */
	COLOR_WHITE,	/* Length 17 */
	COLOR_MAGENTA,	/* Length 19 */
};
int NMoves = 0;

main(int argc, char **argv)
{
	int NTiles, FromCol, ToCol, catch();
	...
	int i;
	for (i = 0; i < 10; i++)
		init_pair(i, TileColour[i], COLOR_BLACK);
	}
	cbreak();
	if(LINES<24) {
...
DisplayTiles()
{
	...
	for(Peg=0; Peg<NPEGS; Peg++) {
		for(SlotNo=0; SlotNo<Pegs[Peg].Count; SlotNo++) {
			...
			attron(COLOR_PAIR(LENTOIND(Pegs[Peg].Length[SlotNo])));
...
Usage()
{
	fprintf(stderr, "Usage: %s [<No Of Tiles>] [a]\n");
