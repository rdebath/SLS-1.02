...
#ifdef TRACE
	if (_tracing)
	    _tracef("newwin: returned window is %x", win);
#endif
...
#ifdef TRACE
	if (_tracing)
	    _tracef("derwin(%d,%d,%d,%d) called", num_lines, num_columns, begy, begx);
#endif
...
#ifdef TRACE
	if (_tracing)
	    _tracef("\tderwin: returned window is %x", win);
#endif
...
#ifdef TRACE
	if (_tracing)
	_tracef("subwin(%x, %d, %d, %d, %d) called", w, l, c, y, x);
#endif
...
#ifdef TRACE
	_tracef("makenew(%d,%d,%d,%d)", num_lines, num_columns, begy, begx);
#endif
