     ...
     int		this_segment_type; /* N_TYPE bits for segment. */
{
  ...
  for ( ;  fixP;  fixP = fixP -> fx_next)
	...
	/* It doesn't. Here is the fix. H.J.
	 */
#if 0
...
#else
	if((size==1 && (add_number& ~0x7F) &&
	   ((add_number&~0x7F)!=(-1&~0x7F))) ||
	   (size==2 && (add_number& ~0x7FFF) &&
	   ((add_number&~0x7FFF)!=(-1&~0x7FFF))))
	  as_bad("Fixup of %d too large for field width of %d",add_number, size);
#endif
