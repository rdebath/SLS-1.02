...
/* This is GNU Info:
   ...
   Version 1.45  (Change "major_version" and "minor_version" below.)
   Fri Feb  7 1992
*/
	  ...
	  if (nodename[temp])
	    {
	      ...
	      while (internal_nodename[tem1++] = nodename[temp++])
		;
	    }
	  else if (*filename != '\0')
     ...
     int offset;
{
  ...
  while (1)
      ...
      if (strnicmp (tag_table + temp, nodename, strlen (nodename)) == 0)
	{
	  ...
	  character = *(tag_table + temp + strlen (nodename));

	  if (character == '\177' || character == ',')
	    return (temp);
	}
    }
}
