...
#ifndef lint
static char rcsid[] = "$Header: /user6/ouster/wish/RCS/tkPack.c,v 1.28 92/12/12 17:32:01 ouster Exp $ SPRITE (Berkeley)";
#endif
				 ...
				 * re-layed out. */
{
	...
	*parentPtr->abortPtr = 1;
    ...
    if (!Tk_IsMapped(parentPtr->tkwin)) {
	return;
    }
	...
	/*
	 * If the window in which childPtr is packed is not its
	 * parent in the window hierarchy, translate the coordinates
	 * to the coordinate system of the real X parent.
	 */

	parent= Tk_Parent(childPtr->tkwin);
	for (ancestor = parentPtr->tkwin; ancestor != parent;
				 ...
				 * against which to pack. */
{
	...
	for (ancestor = parentPtr->tkwin; ; ancestor = Tk_Parent(ancestor)) {
	    if (ancestor == parent) {
		break;
	    }
		...
		Tcl_AppendResult(interp, "can't pack ", argv[0],
			" inside ", Tk_PathName(parentPtr->tkwin),
			(char *) NULL);
		return TCL_ERROR;
	    }
	}
	...
	for (index = 0 ; index < optionCount; index++) {
		...
		if ((Tk_GetPixels(interp, tkwin, options[index+1],
			&packPtr->padX) != TCL_OK) || (packPtr->padX < 0)) {
		    badPad:
		...
		if ((Tk_GetPixels(interp, tkwin, options[index+1],
			&packPtr->padY) != TCL_OK) || (packPtr->padY < 0)) {
		    goto badPad;
		}
		index++;
