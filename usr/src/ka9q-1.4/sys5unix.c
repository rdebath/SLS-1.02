...
unsigned long telmask = 0;
unsigned long tnixmask = 0;
...
eihalt()
{
	...
	again:
	  ...
	  mainmask = selmask | 1;
/*
 * the only signal we use is supposed to be BSD, so we should never
 * get EINTR, but the kernel considers select unrestartable.  Yuck.
 */
	  if (select(32,&mainmask,0,0,&tv) < 0) {
	    if (errno != EINTR)
	  ...
	  if (mainmask & telmask)
	    telservready(mainmask & telmask);
	  if (mainmask & tnixmask)
	    tnix_ready(mainmask & tnixmask);
	}
}
