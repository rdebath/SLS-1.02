        ...
        char **ptyname;
{
	...
	*ptyname = strdup (master);
...
#else
		(*ptyname)[strlen("/dev/pty")] = letters[letcnt];
		(*ptyname)[strlen("/dev/ptyX")] = numbers[numcnt];
#endif
		if (letcnt > letmax) {
			return -1;
	...
	} while ((pty=open(*ptyname, O_RDWR | O_NDELAY)) < 0);

	(*ptyname)[5] = 't'; /* pty to tty */

	ioctl (pty, TCGETS, &tio);
	tio.c_lflag &= ~(ICANON);
	ioctl (pty, TCSETSW, &tio);
	return(pty);
}
