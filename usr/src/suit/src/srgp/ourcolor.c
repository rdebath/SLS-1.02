...
SRGP__initColor (requested_planes)
{
   ...
   if (srgp__available_depth == 1) {
	 ...
	 if (result == 0) {
	     fprintf (stderr, "%s%s%s",
		      "SRGP finds the color table too full to share.  Therefore,\n",
		      "this application will have its own color table.  This will\n",
		      "cause screen colors to change when the mouse enters or leaves the window.\n");
	     /***** APPL WILL HAVE TO SHARE TABLE, EVEN THO IT DIDN'T WANT TO */
	     srgpx__colormap = 
		 XCreateColormap (srgpx__display, 
				  srgp__curActiveCanvasSpec.drawable.win,
				  DefaultVisual(srgpx__display,srgpx__screen),
   ...
   XSetWindowBackground (srgpx__display, 
			 srgp__curActiveCanvasSpec.drawable.win, 0);
   XSetWindowBorder (srgpx__display, 
		     srgp__curActiveCanvasSpec.drawable.win, 1);
}
